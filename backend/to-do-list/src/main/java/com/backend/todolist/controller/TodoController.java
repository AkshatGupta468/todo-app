...
public class TodoController {
	...
	public ResponseEntity<List<Todo>> readAll(Principal principal, @RequestParam(required = false) String tag){

		if(tag != null) {
			return new ResponseEntity<>(todoService.readAllByTag(principal.getName(), tag), HttpStatus.OK);
		}
	...
	@ResponseStatus(code = HttpStatus.OK)
	@RequestMapping(value = "/api/todo/{id}/tag", method = RequestMethod.PUT)
	public ResponseEntity<Todo> setTag(@PathVariable long id, Principal principal,String tag) {
		return new ResponseEntity<>(todoService.setTagById(id, principal.getName(),tag), HttpStatus.OK);
	}
